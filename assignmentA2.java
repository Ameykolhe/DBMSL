...
public class assignmentA2 {
	...
	public assignmentA2(String url, String dbName, String driver, String username, String password){
		this.url = url;
		this.dbName = dbName;
		this.driver = driver;
		...
		try{
			while(choice.equalsIgnoreCase("y")){
				...
				case 1:
					...
					int option9 =st.executeUpdate("CREATE UNIQUE INDEX " + indexName + " ON Professor(phoneno)");
					print("Successful");
					break;
				case 2:
					print("Enter index name: ");
					String indexName2 = sc.next();
					int output1 = st.executeUpdate("CREATE INDEX " + indexName2 + " ON Professor( fName, lName)");
				...
				case 4:
					...
					ResultSet rs1 = st.executeQuery("SELECT * FROM book USE INDEX(tempIndex)");
					while(rs1.next()){
		...
		try{
			while(choice.equalsIgnoreCase("Y")){
				...
				case 1:
					...
					int output = st.executeUpdate("CREATE VIEW " + viewName + " AS SELECT fName,dId from Professor");
					print("Successful");
					break;
				...
				case 2:
					...
					try{
						int output1 = st.executeUpdate("UPDATE " + viewName1 + " SET city = 'Delhi' WHERE pId = 10001");
						print("Successful");
					}
					catch(Exception e){
						e.printStackTrace();
				...
				case 4:
					...
					try{
						...
						while(res.next()){
							System.out.println(res.getString("fName") + " : " + res.getInt("dId"));
						}
					}
					catch( Exception e){
						e.printStackTrace();
				...
				case 5:
					...
					try{
						int output3 = st.executeUpdate("CREATE VIEW " + compview + " AS SELECT Professor.fName,Department.dName FROM Professor natural join Department");
						ResultSet rs = st.executeQuery("SELECT * FROM " + compview);
						while(rs.next()){
							System.out.println(rs.getString("fName") + " : " + rs.getString("dName"));
						}
						print("Successful");
					}
					catch (Exception e){
	...
	public static void main(String[] args) {
		...
		String dbName = "te3144db";
		String driver = "com.mysql.jdbc.Driver";
		String username = "te3144";
		String password = "te3144";
		
		// 
		
		assignmentA2 ob1 = new assignmentA2(url, dbName, driver, username, password);
